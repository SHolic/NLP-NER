...
def train(train_data_path, model_path=None, model_params=None, mode="char", train_test_split_rate=0.8):
    ...
    random_x = x[:5000].copy()
    random_y = y[:5000].copy()
    random_s = sentences[:5000].copy()
    random.seed(123)
    ...
    x_train = random_x[:int(len(random_x) * train_test_split_rate)] + x[5000:]
    x_test = random_x[int(len(random_x) * train_test_split_rate):]
    y_train = random_y[:int(len(random_y) * train_test_split_rate)] + y[5000:]
    y_test = random_y[int(len(random_y) * train_test_split_rate):]
    sentences_train = random_s[:int(len(random_s) * 0.8)] + sentences[5000:]
    sentences_test = random_s[int(len(random_s) * 0.8):]
...
def predict(test_data_path=None, test_data=None, model_path=None, return_type=None, mode="char"):
    ...
    global crf
    global crf_model_path
    if model_path != crf_model_path:
...
class Predictor:
    def __init__(self, model_path):
        self.crf = CRF.load(model_path)

    def predict(self, data=None, data_path=None, mode="char", return_type=None):
        sentences = None
        if data:
            sentences = [data] if isinstance(data, str) else data
            sentences = [clean(sent) for sent in sentences]
            if mode == "word":
                sentences = _trans2word(sentences)
        if data_path:
            sentences = load_test_data(data_path, mode)
